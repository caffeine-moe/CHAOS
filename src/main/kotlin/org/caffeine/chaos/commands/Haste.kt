...
suspend fun haste(client: Client, event: MessageCreateEvent) = coroutineScope {
    if (event.message.content == "${client.config.prefix}haste" && event.message.attachments?.isNotEmpty() == true) {
        event.channel.sendMessage(MessageBuilder()
            ...
            this.launch {
                val response = normalHTTPClient.post("https://www.toptal.com/developers/hastebin/documents") {
                    try {
                        setBody(normalHTTPClient.get(event.message.attachments?.first()?.url.toString()).bodyAsText())
                    } catch (e: Exception) {
                        if (e is MalformedInputException) {
                            message.edit(MessageBuilder()
                                ...
                                .build()).thenAccept { this@coroutineScope.launch { onComplete(it, client, true) } }
                            return@launch
                        }
                    }
                }
    ...
    if (event.message.content.startsWith("${client.config.prefix}haste ") && event.message.content != "${client.config.prefix}haste " && event.message.attachments?.isEmpty() == true) {
        event.channel.sendMessage(MessageBuilder()
            .appendLine("Creating haste...")
            .build()).thenAccept { message ->
            this.launch {
                val response = normalHTTPClient.post("https://www.toptal.com/developers/hastebin/documents") {
                    setBody(event.message.content.removePrefix("${client.config.prefix}haste "))
                }
                val haste = json.decodeFromString<HasteResponse>(response.bodyAsText())
                message.edit(MessageBuilder()
                    .appendLine("https://www.toptal.com/developers/hastebin/${haste.key}")
